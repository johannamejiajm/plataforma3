...
@section('contenido')
            ...
            @if ($historias)
                @foreach ($historias as $key => $historia)
                    <div class="card-historia">
                        <div class="card-historia-img" data-bs-toggle="modal" data-bs-target="#modalHistoria{{ $key }}"
                        ...
                        <div class="card-historia-body">
                            ...
                            <p class="descripcion">Publicado el
                                {{ \Carbon\Carbon::parse($historia->fechainicial)->format('d \\d\\e F \\d\\e Y') }}
                            </p>
                            <a data-bs-toggle="modal" data-bs-target="#modalHistoria{{ $key }}" class="leer-mas">Leer más <i
    ...
    @if ($historias)
        @foreach ($historias as $key => $historia)
            ...
            <div class="modal fade" id="modalHistoria{{ $key }}" tabindex="-1" aria-labelledby="modalLabel{{ $key }}"
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body p-5">
                            <div class=" flex-wrap justify-content-between mb-3">
                                <div class="tarjeta-barra">
                                    ...
                                    <div class="tarjeta-info-mini">
                                        ...
                                        <strong>Fecha:</strong>
                                        {{ \Carbon\Carbon::parse($historia->fechainicial)->format('d/m/Y') }}
                                    </div>
                                </div>
                                    ...
                                    <div class="carousel-inner">
                                        @foreach ($historia->fotos as $index => $foto)
                                            <div class="carousel-item @if($index === 0) active @endif">
                                                <img src="{{ asset($foto->imagen) }}" class="d-block w-100"
                                                    alt="Imagen de la galería de {{ $historia->titulo }}">
                                            </div>
                                        @endforeach
                                    </div>
                            ...
                            <div class="d-flex gap-3 flex-wrap mt-3">
                                ...
                                <!-- Botón WhatsApp -->
                                <a href="https://wa.me/3147949465" class="btn btn-success d-flex align-items-center gap-2"
                                    target="_blank" aria-label="Contáctanos por WhatsApp">
                            ...
                            <div class="modal-footer">
                                <small class="text-muted">Publicado:
                                    {{ \Carbon\Carbon::parse($historia->fechainicial)->format('d/m/Y') }}</small>
